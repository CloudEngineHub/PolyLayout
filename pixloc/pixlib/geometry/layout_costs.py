...
class FeaturemetricCost:
    ...
    def residuals(
        ...
        if isinstance(layout, Polygon):  # Check for occlusion
            p3D_proj, _ = layout.unsqueeze(-1).project(camera.normalize(p2D), T_w2cam)
            p3D_proj = T_w2cam * p3D_proj
            visible = p3D_proj[..., 2] > p3D_cam[..., 2] - self.eps
        ...
        # Don't warp from or to cameras outside the room layout
    ...
    def residual_jacobian(
        ...
        res, valid, weight, info = self.residuals(
            layout, T_w2cam, camera, p2D, F, F_interp, W, W_interp, True)
...
class EdgeCost:
    ...
    def residuals(
        ...
        if isinstance(layout, Cuboid):
            p3D_w, J_p3Dw_Rd = layout.unsqueeze(-1).sample_edges(
                self.num_points, return_jacobian=do_gradients)
        else:  # Polygon
            p3D_w, sampled, J_p3Dw_Rd = layout.unsqueeze(-1).sample_edges(
                self.num_points, return_jacobian=do_gradients)
        p3D_cam = T_w2cam * p3D_w
    ...
    def residual_jacobian(
        ...
        res, valid, weight, info = self.residuals(
            layout, T_w2cam, camera, E, W, True)
        J = self.jacobian(T_w2cam, camera, *info)
        return res, valid, weight, J
...
class VanishingPointCost:
    ...
    def residuals(self, layout: Layout, T_w2cam: Pose, l2D: Tensor):
        ...
        vp3D = layout.R @ T_w2cam.R.transpose(-1, -2)
    ...
    def jacobian(
        ...
        J_dist_R = J_dist_line3D @ J_line3D_vp3D @ J_vp3D_R.unsqueeze(-4)
        J_dist_R = J_dist_R.squeeze(-2)

        mask = dist_signed.abs() > self.dist_thr
        J_dist_R.masked_fill_(mask.unsqueeze(-1), 0.0)

        idx = idx.unsqueeze(-1).unsqueeze(-1).repeat((1,) * idx.ndim + (1, 3))
        J_dist_R = torch.gather(J_dist_R, -2, idx).squeeze(-2)

        return J_dist_R

    def residual_jacobian(self, layout: Layout, T_w2cam: Pose, l2D: Tensor):
        res, info = self.residuals(layout, T_w2cam, l2D)
